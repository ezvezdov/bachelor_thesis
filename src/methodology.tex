...
These models leverage the \ac{BERT} and A Lite \ac{BERT} (ALBERT) \cite{ALBERT} architectures and are designed to outperform multilingual models by training exclusively on Czech data.
...
For the embedding generation stage within \ac{RAG}, we will leverage the GTE-v1.5\textsubscript{Large} model.
This selection is based on its performance as the highest-scoring transformer model for English according to the \ac{MTEB} results.
